...
best_score, bcfg = batch([(PI/2, -PI/2, 0.0)], "baseline")[0]
print(f"Baseline: {100*best_score:.2f}%\n")
...
for rnd in (1, 2):
    ...
    for gi, name in enumerate(["theta_przod", "theta_tyl", "phi"]):
        ...
        per_knob[gi] = sorted({x[1][gi] for x in r[:2]} | {bcfg[gi]})
        if r[0][0] > best_score + 1e-4:
            best_score, bcfg = r[0]
            improved = True
            print(f"  -> poprawa! {100*best_score:.2f}%")
        else:
            print(f"  -> spadek, cofam (zostaje {tuple(round(x,3) for x in bcfg)})")
    mixes = [tuple(per_knob[i][mi] for i in range(3))
             for mi in __import__('itertools').product(range(len(per_knob[0])), repeat=3)][:12]
    ...
    if not improved:
        print("Wynik przestal rosnac -> szcyt osiagniety")
        break

print(f"\n=== SZCZYT 32q: {100*best_score:.2f}% @ {tuple(round(x,4) for x in bcfg)} ===")
