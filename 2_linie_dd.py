...
print(f"\n=== 2 LINIE + DYNAMICAL DECOUPLING 32q ===")
print(f"Głębokość: {qc_t.depth()}")

# DD - dynamical decoupling (XpXm sekwencja)
...
print(f"\n{'=' * 60}")
print(f"2 LINIE + DYNAMICAL DECOUPLING 32q")
print(f"{'=' * 60}")
print(f"Echo |0...0>: {zero}/{total} = {zero/total*100:.1f}%")
print(f"\nBez DD: 87.5%")
print(f"Z DD:   {zero/total*100:.1f}%")

if zero/total > 0.875:
    print(f"\n🎉 DD DZIAŁA! +{zero/total*100 - 87.5:.1f}%!")
else:
    print(f"\n❌ Spadek. 87.5% bez DD to był rekord.")
