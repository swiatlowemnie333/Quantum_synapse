...
def build_path(banned):
    ...
    for start in team:
        ...
        # odwroc i probuj dokleic z drugiej strony
...
for wave_nr in (1, 2, 3):
    ...
    history.append(avg)
    print(f"Srednia fidelity: {100*avg:.2f}%")
    per_pos.sort()
    print("Najslabsze 3:", [(f"p{i} q{q}", f"{100*f:.0f}%") for f, i, q in per_pos[:3]])
    weak = [q for f, i, q in per_pos[:3] if f < 0.95]
    if not weak:
        print("Wszystkie pozycje >= 95% -> szczyt osiagniety, stop")
        break
    banned.update(weak)
    print(f"Wymieniam {len(weak)} slabe -> nastepna fala")

print("\n=== HISTORIA ===")
for i, a in enumerate(history, 1):
    print(f"Fala {i}: {100*a:.2f}%")
