...
tries = 0
while len(best) < TARGET and tries < 15000:
    tries += 1
    ...
    while True:
        ...
        u = nxt[0] if random.random() < 0.7 or len(nxt) == 1 else random.choice(nxt[:3])
        path.append(u); seen.add(u)
    if len(path) > len(best): best = path
    if tries % 3000 == 0:
        print(f"proba {tries}, najdluzsza: {len(best)}")

path = best[:TARGET]
K = len(path)
print(f"Linia z mostami: {K} kubitow (prob: {tries})")
print(path)
print(f"Srednie T1={sum(t1[q] for q in path)/K:.0f} T2={sum(t2[q] for q in path)/K:.0f}")
bridges = [q for q in path if q in (67, 47)]
print(f"Mosty w linii: {bridges}")
