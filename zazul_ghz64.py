...
for t, pub in zip(thetas, r):
    counts = pub.data.meas.get_counts()
    z = counts.get("0"*K, 0)/SHOTS
    o = counts.get("1"*K, 0)/SHOTS
    s = z + o
    if s > best[0]: best = (s, t)
    print(f"theta={t:.3f}: |0..0> {100*z:.2f}% + |1..1> {100*o:.2f}% = {100*s:.2f}%")
print(f"\n=== GHZ 64q NAJLEPSZE: {100*best[0]:.2f}% @ theta={best[1]:.3f} ===")
print("(GHZ 32q bylo 32.7%)")
