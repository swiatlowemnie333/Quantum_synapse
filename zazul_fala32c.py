...
tries = 0
while len(best) < 32 and tries < 5000:
    tries += 1
...
path = best[:32]
K = len(path)
print(f"Linia bez slabych ogniw: {K} kubitow (prob: {tries})")
print(path)
...
per_pos.sort()
print(f"\n=== FALA 1010 na {K}q po dokreceniu: {100*avg:.2f}% (bylo 97.18%) ===")
