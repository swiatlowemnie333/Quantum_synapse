...
with open("sonda_36_IONQ.qasm", "rb") as f:
    qasm_bytes = f.read()

print("=== KALIBRACJA 36q IONQ (auto execution plan) ===")
config = JobSubmissionConfig(
    organization_id=org_id,
    backend_class_id="ionq:forte-ent",
    name="Zazul Kalibracja 36q",
    job_subcategory_id="phys:hds",  # Hamiltonian Dynamics - pasuje do fali
    shots=8192,
    execution_plan="auto",  # <-- AUTO zamiast PUBLIC
    queue_priority="auto",
...
if job.status == 'COMPLETED':
    ...
    print(result)
    import json
    with open('kalibracja_ionq_36q.json', 'w') as f:
        json.dump(result, f, indent=2)
    print("Zapisano: kalibracja_ionq_36q.json")
else:
